...
<head>
    <title><fmt:message key="instock.baseline.list.title"/></title>
    <meta name="menu" content="instockBaselineMenu"/>
	<script src="<c:url value='/themes/admin/global_assets/js/plugins/forms/validation/validate.min.js'/>"></script>
	<link href="<c:url value='/themes/admin/assets/css/custom_style.css'/>" rel="stylesheet" type="text/css">
	<script src="<c:url value='/themes/admin/assets/js/instock_baseline_list.js'/>"></script>
...
<div class="content">
	<div class="row cus-tabs-container mb-3">
		<ul class="nav nav-tabs mb-0">
			...
			<li class="nav-item"><a href="/instock-baseline/list" class="nav-link active show"><fmt:message key="menu.instock.baseline"/></a></li>
		</ul>
		<div class="tab-divider"></div>
	</div>
	<div class="row mb-3">
		<div class="col-12 col-md-6">
			<div class="text-color-primary fs-5"><fmt:message key="instock.baseline.list.title" /></div>
		</div>
	</div>
<form:form id="instockBaselineForm" modelAttribute="criteria" action="/instock-baseline/list" method="post">
			...
			<table class="table table-bordered table-striped">
				<thead>
					<tr class="table-success">
						...
						<th class="bgc-primary border-primary text-white"><fmt:message key="instock.syncDate"/></th>
						...
						<tr>
							...
							<td class="text-center">
								<div class="list-icons">
									<security:authorize access="hasAnyRole('ROLE_ADMIN,ROLE_INSTOCK_BASELINE_EDIT')">
									<a href="/instock-baseline/form?id=${item.id}"><i class="icon-file-eye fontSize18" title="Chi tiết"></i></a>
									</security:authorize>
								</div>
							</td>
						</tr> 
