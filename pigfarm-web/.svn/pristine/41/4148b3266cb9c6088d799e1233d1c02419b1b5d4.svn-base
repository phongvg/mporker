...
<div class="content">
	<div class="row mb-3">
		<div class="col-12 col-md-6">
			<div class="text-color-primary fs-3"><fmt:message key="closeAttrition.list.title" /></div>
		</div>
		<div class="col-12 col-md-6 text-right">
			<security:authorize access="hasAnyRole('ROLE_ADMIN','ROLE_PERIOD_CLOSE')">
				<a href="<c:url value='/goodsAttristionClose/form'/>" class="btn bgc-warning text-white" title="<fmt:message key="button.add"/>"><i class="icon-plus22"></i><fmt:message key="button.add"/></a>
			</security:authorize>
		</div>
	</div>
<!-- \Table -->
				...
				<c:forEach var="item" items="${list}" varStatus="cnt">
					<tr>
						...
						<td class="text-center">
							<button type="button" id="delete_${item.id }" class="btn btn-sm bgc-warning deleteException"><i class="icon-bin"></i></button>
						</td>
					</tr> 
				</c:forEach>
			</tbody> 
