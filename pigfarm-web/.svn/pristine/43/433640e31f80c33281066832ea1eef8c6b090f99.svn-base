...
<div class="task--column plan">
  <div class="task--column__title">
    <span><fmt:message key="task.view.mode.table.title.plan"/></span>
  </div>
  <div class="task--column__data">
    <c:forEach var="task" items="${criteria.plans}">
  ...
  <c:if test="${criteria.totalPlanElements > 5}">
    <div class="row">
      <div class="col-12 text-center">
        <span class="text-link" id="task--plan__showMore">Show more</span>
      </div>
    </div>
  </c:if>
</div>
