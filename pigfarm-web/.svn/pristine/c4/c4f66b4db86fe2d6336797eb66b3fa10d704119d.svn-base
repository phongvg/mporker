...
<div class="task--column rejected">
  <div class="task--column__title">
    <span><fmt:message key="task.view.mode.table.title.rejected"/></span>
  </div>
  <div class="task--column__data">
    <c:forEach var="task" items="${criteria.rejects}">
  ...
  <c:if test="${criteria.totalRejectedElements > 5}">
    <div class="row">
      <div class="col-12 text-center">
        <span class="text-link" id="task--rejected__showMore">Show more</span>
      </div>
    </div>
  </c:if>
</div>
