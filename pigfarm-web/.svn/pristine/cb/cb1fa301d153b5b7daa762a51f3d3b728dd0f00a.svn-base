var ReportWeightCompareComponent = function() {
	...
	var _handleExportReportInstock = function() {
        $('#exportReportInstock').on('click', function(e) {
            ...
            var apiExport;
            if(barnCodesStr != ''){
				apiExport = '/report/weight-compare-export-excel?stockCodes=' + stockCodeStr + '&stage=' + stage+ '&barnCodes='+barnCodesStr;
			} else {
				apiExport = '/report/weight-compare-export-excel?stockCodes=' + stockCodeStr + '&stage=' + stage;
			}
