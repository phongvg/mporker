...
public class ProcessOrderController extends BaseController {
    ...
    public ModelAndView show(@RequestParam(value="id", required=false) Long id, HttpServletRequest request) {
		...
		Integer totalPigRetained = 0;
		if(id != null) {
			processOrderDto = processOrderManager.get(id);
			if (processOrderDto == null) {
				addError(request, getText("data.notfound", locale));
				return new ModelAndView("redirect:/processOrder/list");
			}
			totalPigRetained = productionManager.getTotalPigRetainedByPoCode(processOrderDto.getCode());
		}
