...
public class BarnPlanManager {
	...
	public EntityResponse confirm(BarnPlanDto barnPlanDto) {
		...
		if(barnPlanDto.getId() != null) {
			response = restHelper.send(serviceProperties.getPaths().get(ApiEnum.PATH_BARN_PLAN_CONFIRM.val), HttpMethod.PUT, barnPlanDto, EntityResponse.class, barnPlanDto.getId());
		} else {
			response = restHelper.send(serviceProperties.getPaths().get(ApiEnum.PATH_BARN_PLAN_CREATE.val), HttpMethod.POST, barnPlanDto, EntityResponse.class);
		}
