...
public class FarmManager {
	...
	public EntityResponse save(FarmDto farmDto) {
		log.info("PROCESS: SAVE FARM, FARM: {}", farmDto);
		ResponseEntity<?> response = null;
		if (farmDto.getId() != null) {
			response = restHelper.send(serviceProperties.getPaths().get(ApiEnum.PATH_FARM_UPDATE.val), HttpMethod.PUT, farmDto, EntityResponse.class, farmDto.getId());
		} else {
			response = restHelper.send(serviceProperties.getPaths().get(ApiEnum.PATH_FARM_CREATE.val), HttpMethod.POST, farmDto, EntityResponse.class);
		}
