...
public class ProcessOrderManager {
	...
	public EntityResponse confirm(ProcessOrderDto processOrder) {
		...
		if(processOrder.getId() != null) {
			response = restHelper.send(serviceProperties.getPaths().get(ApiEnum.PATH_PROCESS_ORDER_CONFIRM.val), HttpMethod.PUT, processOrder, EntityResponse.class, processOrder.getId());
		} else {
			response = restHelper.send(serviceProperties.getPaths().get(ApiEnum.PATH_PROCESS_ORDER_CREATE.val), HttpMethod.POST, processOrder, EntityResponse.class);
		}
