...
var Component = function() {
	...
	var confirmPO = function(){
		$('#processOrderConfirm').on('click',function(e){
			bootbox.confirm({
				title: "Xác nhận:",
			    message: "Bạn có chắc chắn muốn lưu lệnh không?",
			    buttons: {
			        confirm: {
			            label: 'Xác nhận',
			    ...
			    callback: function (result) {
			    	if(result) {
						...
						var startDate = $('#processOrderStartDate').val();
						if(startDate) {
							var strS = startDate.split('/');
							var monthS = Math.abs(strS[1]) - 1; //// the month is 0-indexed
							var value = new Date(strS[2],monthS,strS[0]).toLocaleDateString('en-GB');
							document.getElementById("startDate").value = value;
						}

						//set lai ngay thang nam
						var endDate = $('#processOrderEndDate').val();
						if(endDate) {
							var strE = endDate.split('/');
							var monthE = Math.abs(strE[1]) - 1; //// the month is 0-indexed
							var enddvalueate = new Date(strE[2],monthE,strE[0]).toLocaleDateString('en-GB');
